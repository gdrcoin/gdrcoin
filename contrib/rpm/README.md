...
The RPM spec file provided here is for Gdrcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gdrcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gdrcoin git repository](https://gitlab.com/gdrcoin/gdrcoin) but are not part of
...
Currently only `Source10` is used, for the example `gdrcoin.conf` file.
...
in the gdrcoin git repository. At present only an SVG version of the Gdrcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gdrcoin
...
needed when 0.12.1 is released, a proper fix is already in the Gdrcoin git
master branch.
...
idea to build Gdrcoin Core against a static version of that library compiled
...
The `gdrcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gdrcoin-server` package. At this point, most RPM based
...
The `gdrcoin-server` package should have SELinux support. How to properly do
...
Gdrcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gdrcoin/). He has been
packaging Gdrcoin for Fedora at least since 2012.
...
4. I renamed the `gdrcoin` package that contains the Qt GUI to `gdrcoin-core` as
that appears to be how the general population refers to it, in contrast to
`gdrcoin-xt` or `gdrcoin-classic`. I wanted to make sure the general population

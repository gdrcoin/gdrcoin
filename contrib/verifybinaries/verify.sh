...
###   core.gdrcoin.org and gdrcoin.org and compares them.
...
WORKINGDIR="/tmp/gdrcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://core.gdrcoin.org"
HOST2="https://gdrcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="gdrcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gdrcoin-',
...
if [ $? -ne 0 ]; then
   echo "gdrcoin.org failed to provide signature file, but core.gdrcoin.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "gdrcoin.org and core.gdrcoin.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gdrcoin Core binary release signing key installed?"
   fi

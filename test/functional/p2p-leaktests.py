...
Also test that nodes that send unsupported service bits to gdrcoind are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if gdrcoind ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(GdrcoinTestFramework):

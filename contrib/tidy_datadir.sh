#!/bin/bash
# Copyright (c) 2013 The Gdrcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Gdrcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Gdrcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Gdrcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Gdrcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Gdrcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Gdrcoin 0.8 datadir."
    ;;
esac
